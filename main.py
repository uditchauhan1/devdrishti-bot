...
class DevDrishtiRazorpayBot:
    ...
    async def handle_admin_commands(self, message):
        ...
        try:
            ...
            if message.text and len(message.text.split()) > 1:
                ...
                if command == 'pending':
                    await self.show_pending_payments()
                    return
                elif command == 'status':
            ...
            admin_msg = (
                ...
                f"/admin pending - Show pending payments\n"
                f"/admin status - Show detailed status\n\n"
    ...
    async def show_pending_payments(self):
        """Show pending payments to admin"""
        try:
            if not self.pending_payments:
                await self.bot.send_message(self.admin_user_id, "✅ No pending payments at the moment.")
                return
            
            msg = "📋 **Pending Payment Verifications:**\n\n"
            for user_id, payment_info in self.pending_payments.items():
                timestamp = payment_info['timestamp'].strftime('%H:%M:%S')
                amount = payment_info['expected_amount']
                msg += f"👤 User {user_id}\n💰 ₹{amount}\n⏰ {timestamp}\n\n"
            
            await self.bot.send_message(self.admin_user_id, msg)
            
        except Exception as e:
            self.logger.error(f"Error showing pending payments: {e}")
    
    async def show_bot_status(self):
        """Show detailed bot status to admin"""
        try:
            msg = (
                "🤖 **Detailed Bot Status**\n\n"
                f"🔐 Admin ID: {self.admin_user_id}\n"
                f"💳 UPI ID: {self.upi_id}\n"
                f"🏦 Razorpay: {'Enabled' if self.razorpay_enabled else 'Disabled'}\n"
                f"📊 User States: {len(self.user_states)} active\n"
                f"💰 User Pricing: {len(self.user_pricing)} assigned\n"
                f"⏳ Pending Payments: {len(self.pending_payments)}\n"
                f"📈 Referrals: {sum(self.user_referrals.values())}\n\n"
                f"🎯 **Pricing Variants:** ₹{self.pricing_variants}\n"
                f"⚡ **Rate Limit:** {self.rate_limit_seconds}s between calls\n"
                f"📈 **Max Requests:** {self.max_requests_per_hour}/hour"
            )
            await self.bot.send_message(self.admin_user_id, msg)
    ...
    async def handle_callback_query(self, call):
        ...
        try:
            ...
            # Answer callback immediately to prevent multiple clicks
            await self.bot.answer_callback_query(call.id, "⏳ Processing...")
            ...
            if user_id not in self.pending_payments:
                ...
                already_processed_msg = (
                    "⚠️ **ALREADY PROCESSED**\n\n"
                    "This payment verification has already been handled.\n"
                ...
                try:
                    ...
                    self.logger.warning(f"Could not edit admin message: {edit_error}")
                    # Send new message if editing fails
                    await self.bot.send_message(call.message.chat.id, already_processed_msg)
                return
    ...
    async def approve_payment(self, user_id, call):
        ...
        try:
            self.logger.info(f"🔄 Processing approval for user {user_id}")
            
            if user_id in self.pending_payments:
                # Remove from pending
                payment_info = self.pending_payments.pop(user_id)
                self.logger.info(f"✅ Payment info retrieved and removed from pending for user {user_id}")
                
                # Create success message
                success_msg = "✅ **PAYMENT APPROVED**\n\n" + payment_info['admin_msg'] + "\n\n🎯 **Status:** Approved by admin\n⏰ Processing PDF generation..."
                
                # Update admin message (handle both text and photo messages)
                try:
                    if call.message.photo:
                        # Message has photo - edit caption
                        await self.bot.edit_message_caption(
                    ...
                    self.logger.warning(f"Could not edit admin message: {edit_error}")
                    # Send new message if editing fails
                    await self.bot.send_message(call.message.chat.id, success_msg)
                ...
                self.logger.info(f"✅ Payment approval completed for user {user_id}")
                
            else:
                self.logger.warning(f"⚠️ Payment for user {user_id} not found in pending list - already processed?")
                # This case is now handled in handle_callback_query, so this shouldn't happen
                
        except Exception as e:
            self.logger.error(f"❌ Error approving payment for user {user_id}: {e}")
            await self.bot.send_message(self.admin_user_id, f"❌ Error processing approval for user {user_id}: {str(e)}")
    
    async def reject_payment(self, user_id, call):
        """Handle payment rejection by admin"""
        try:
            self.logger.info(f"🔄 Processing rejection for user {user_id}")
            ...
            if user_id in self.pending_payments:
                ...
                rejection_msg = "❌ **PAYMENT REJECTED**\n\n" + payment_info['admin_msg'] + "\n\n🚫 **Status:** Rejected by admin"
                ...
                try:
                    ...
                    self.logger.warning(f"Could not edit admin message: {edit_error}")
                    # Send new message if editing fails
                    await self.bot.send_message(call.message.chat.id, rejection_msg)
                ...
                self.logger.info(f"❌ Payment rejection completed for user {user_id}")
            ...
            self.logger.error(f"❌ Error rejecting payment for user {user_id}: {e}")
            await self.bot.send_message(self.admin_user_id, f"❌ Error processing rejection for user {user_id}: {str(e)}")
    ...
    async def handle_payment_confirmation(self, user_id, message):
        ...
        try:
            ...
            if self.admin_user_id:
                ...
                self.logger.info(f"🔐 Sending admin verification request to admin {self.admin_user_id}")
                await self.send_admin_verification_request(user_id, message, user_price)
            else:
                # No admin set - auto-approve (fallback)
                self.logger.warning(f"⚠️ No admin set (ADMIN_USER_ID not configured) - auto-approving payment for user {user_id}")
    ...
    async def handle_photo(self, message):
        ...
        try:
            ...
            if self.user_states[user_id] == 'payment_pending':
                # Use the new admin verification system
